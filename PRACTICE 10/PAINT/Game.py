...
class PaintApp:
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif event.key == pygame.K_2: self.tool = 'rect'
                elif event.key == pygame.K_3: self.tool = 'circle'
                elif event.key == pygame.K_4: self.tool = 'eraser'
    ...
    def draw_shape(self, surface, start, end):
        """Метод для отрисовки прямоугольников и кругов."""
        x1, y1 = start
        x2, y2 = end
        
        if self.tool == 'rect':
            # Рассчитываем координаты верхнего левого угла и размеры
            rect_x = min(x1, x2)
            rect_y = min(y1, y2)
            rect_w = abs(x1 - x2)
            rect_h = abs(y1 - y2)
            pygame.draw.rect(surface, self.color, (rect_x, rect_y, rect_w, rect_h), self.radius // 2 + 1)
            
        elif self.tool == 'circle':
            # Считаем расстояние между точками как радиус
            dist = int(((x1 - x2)**2 + (y1 - y2)**2)**0.5)
            pygame.draw.circle(surface, self.color, start, dist, self.radius // 2 + 1)
    ...
    def draw(self):
        ...
        if self.drawing and self.tool in ['rect', 'circle']:
